...
int main(int argc, char *argv[]) {
    ...
    // Mean is the sum divided by the number of numbers in list.
    mean /= n;

    // Median

    // Initialize median to 0.0
    double median = 0.0;
    ...
    // Initialize mode and max_count to 0
    int mode = 0;
    int max_count = 0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Update mode and max_count if the current count is greater than max_count
        if (count > max_count || (count == max_count && nums[i] < mode)) {
            max_count = count;
            mode = nums[i];
        }
    }
